...
const GraphVisualizer = ({ variables }) => {
  ...
  const getNodeFill = (nodeId) => {
    ...
    if (nodeId === current_node) return "rgba(214, 39, 40, 1)";
    const frontier = queue.length > 0 ? queue : stack;
    if (Array.isArray(frontier) && frontier.includes(nodeId)) return "rgba(255, 215, 0, 1)";
    if (visited.includes(nodeId)) return "rgba(255, 127, 14, 1)";
...
const handleExport = async () => {
  try {
    ...
    if (!element) {
      alert("Visualization not found on screen!");
      return;
    }

    console.log("Starting export using dom-to-image-more...");

    const dataUrl = await toPng(element, {
      quality: 1,
      cacheBust: true,
      bgcolor: "#0f172a", // keeps dark background
    });
    ...
    console.log("✅ Export complete without oklab error!");
  } catch (error) {
    console.error("❌ Export failed:", error);
    alert("Failed to export visualization: " + (error.message || "Unknown reason"));
  }
};
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-900 to-black text-white flex flex-col items-center py-10 px-8 overflow-hidden">
      ...
      >
        ...
        <div
          ...
          style={{ width: `${100 - editorWidth}%`, position: "relative" }}
  ...
  <button
    ...
    className="flex items-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-5 py-2 rounded-xl font-semibold shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-200"
  >
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M12 12V4m0 8l-3-3m3 3l3-3" />
    </svg>
    Export Visual
  </button>
